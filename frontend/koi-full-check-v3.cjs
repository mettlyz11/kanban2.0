...
(async () => {
  ...
  for (let i = 0; i < ROUTES.length; i++) {
    ...
    if (result.ok) { results.summary.pass++; process.stdout.write('OK\n'); }
    else if (result.isLoginPage) { results.summary.login++; process.stdout.write('LOGIN\n'); }
    else { results.summary.fail++; process.stdout.write('FAIL ' + (result.error || '').substring(0,30) + '\n'); }
  ...
  console.log('\n========================================');
  console.log('KOI Full Check Report');
  console.log('========================================');
  ...
  if (failedPages.length > 0) {
    console.log('\nFailed pages:');
    failedPages.forEach(r => console.log('  - ' + r.route + ': ' + (r.error || 'No content')));
  }
  ...
  fs.writeFileSync('/opt/kanban-react/qa-reports/full-check-v3.json', JSON.stringify(results, null, 2));
