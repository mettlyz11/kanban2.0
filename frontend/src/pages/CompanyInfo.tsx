...
export function CompanyalInfo() {
  ...
  useEffect(() => {
    if (id) {
      setLoading(true)
      fetchCompanyDetail(parseInt(id)).finally(() => {
        setLoading(false)
      })
    } else {
      setLoading(false)
  ...
  const fetchCompanyDetail = async (companyId: number) => {
    try {
      const res = await fetch(`/api/company-info/companies/${companyId}`)
      const data = await res.json()
      if (data.success) { 
        setCompany(data.company); 
        fetchCompanyTabs(companyId);
      }
    } catch (e) {
  ...
  const fetchCompanyTabs = async (companyId: number) => {
    try {
      const res = await fetch(`/api/companies/${companyId}/tabs`)
      const data = await res.json()
      if (data.success) {
        setTabs(data.tabs)
  ...
  const toggleFavorite = async () => {
    if (!company) return
    try {
      await fetch(`/api/companies/${company.id}/favorite`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ is_favorite: !company.is_favorite })
      })
      setCompany({ ...company, is_favorite: !company.is_favorite })
    } catch (e) {
      console.error('切换公司收藏失败:', e)
    }
  }

  const createTab = async () => {
    if (!newTabName.trim() || !company) return
    try {
      await fetch(`/api/companies/${company.id}/tabs`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      setNewTabName('')
      setShowAddTab(false)
      await await fetchCompanyTabs(company.id)
    } catch (e) {
      console.error('创建公司标签页失败:', e)
    }
  }

  const deleteTab = async (tabId: number) => {
    if (!company || !confirm('确定要删除这个标签页吗？')) return
    try {
      await fetch(`/api/companies/${company.id}/tabs/${tabId}`, { method: 'DELETE' })
      fetchCompanyTabs(company.id)
      if (activeTabId === tabId) setActiveTabId(null)
    } catch (e) {
      console.error('删除公司标签页失败:', e)
    }
  }
  ...
  const createItem = async (tabId: number) => {
    if (!company) return
    try {
      await fetch(`/api/companies/${company.id}/tabs/${tabId}/items`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title: '新项目', content: '', item_date: new Date().toISOString().split('T')[0] })
      })
      await fetchCompanyTabs(company.id)
    } catch (e) {
      console.error('创建公司项目失败:', e)
    }
  }
  ...
  const saveItem = async () => {
    if (!company || !activeTabId || !editingItem) return
    try {
      ...
      const res = await fetch(`/api/companies/${company.id}/tabs/${activeTabId}/items/${editingItem.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title: editTitle, content: editContent, item_date: formattedDate, attachments: editAttachments })
      })
      const data = await res.json()
      if (data.success) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      formData.append('entity_id', company.id.toString())
  ...
  const deleteItem = async (tabId: number, itemId: number) => {
    if (!company || !confirm('确定要删除这个项目吗？')) return
    try {
      await fetch(`/api/companies/${company.id}/tabs/${tabId}/items/${itemId}`, { method: 'DELETE' })
      await fetchCompanyTabs(company.id)
    } catch (e) {
      console.error('删除公司项目失败:', e)
    }
  }

  if (loading) return <div className="p-8 text-center">加载中...</div>
  if (!company) return <div className="p-8 text-center">未找到公司信息</div>
    ...
    <div className="max-w-6xl mx-auto p-6">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <button onClick={toggleFavorite} className="p-1 hover:bg-white rounded">
                  <Star className={`w-6 h-6 ${company.is_favorite ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`} />
                </button>
              </div>
              <p className="text-[rgba(0,0,0,0.8)]">{company.industry}</p>
              <p className="text-sm text-[rgba(0,0,0,0.48)]">{company.email} {company.phone && `· ${company.phone}`}</p>
            </div>
          </div>
          <button onClick={() => navigate("/company")} className="flex items-center gap-1 px-4 py-2 text-[rgba(0,0,0,0.8)] hover:bg-white rounded">
      ...
      {showAddTab && (
        <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-[#f5f5f7] rounded-xl p-6 mx-auto w-[95vw] max-w-4xl">
