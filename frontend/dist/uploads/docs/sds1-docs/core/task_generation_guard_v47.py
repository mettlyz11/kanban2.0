...
│  - 每目标(goal_id)每24小时最多2个任务（Task#2119优化）│
│  - 滑动窗口计数，精确到秒                             │
...
│  - 相似度阈值0.85（Task#2119优化，严格去重）          │
│  - 标准化文本处理（去除标点、空格、大小写）           │
└─────────────────────────────────────────────────────┘

创建日期: 2026-04-26
更新日期: 2026-04-28 (Task#2119最终优化)
版本: V4.7-2119-Production
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / 'sds-task-generation-guard-v47.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('TaskGenerationGuardV47')
...
class IdempotencyLayer:
    ...
    def __init__(self, log_file: str = None):
        self.log_file = log_file or str(LOG_DIR / 'sds-idempotency-v47.log')
...
class RateLimitLayer:
    ...
    DEFAULT_MAX_TASKS_PER_24H = 2  # Task#2119: 从10降至2
    DEFAULT_MAX_PENDING_PER_GOAL = 3  # Task#2119: 从15降至3
...
class SemanticDedupLayer:
    ...
    DEFAULT_SIMILARITY_THRESHOLD = 0.85  # Task#2119: 从0.45提升至0.85
...
class TaskGenerationGuard:
    ...
    def __init__(self,
        ...
        logger.info(f"TaskGenerationGuard V4.7 initialized, batch_id={self.batch_id}")
    
    def _generate_batch_id(self) -> str:
        return f"V47-{datetime.now().strftime('%Y%m%d-%H%M%S')}-{os.urandom(4).hex()}"
    ...
    def create_task_safely(self, title: str, description: str,
                           ...
                           priority: int = 2,
                           task_type: str = 'auto_generated_v4.7',
    ...
    def filter_recommendations(self, recommendations: List[Dict]) -> Tuple[List[Dict], List[Dict]]:
        ...
        logger.info(f"开始V4.7过滤: 共 {len(recommendations)} 个推荐任务")
        ...
        logger.info(f"V4.7过滤完成: 通过 {len(passed)}, 阻止 {len(blocked)}")
