...
"""
更新看板 3.0 个人信息和公司信息
- 更新刘宇宙个人信息（contacts, entities）
- 更新公司信息（company_info）
- 添加教育背景、工作经历、奖项、论文、专利
"""
...
def update_liuyuzhou_contact():
    """更新 contacts 表中刘宇宙的信息"""
    ...
    cursor.execute('''
        UPDATE contacts 
        SET phone = ?, email = ?, department = ?, company = ?
        WHERE name = '刘宇宙'
    ''', (
        '+86-138-xxxx-xxxx',  # phone - 待确认
        'liuyuzhou@buaa.edu.cn',  # email
        '化学学院',  # department
        '北京航空航天大学'  # company
    ))
    
    conn.commit()
    print(f"✓ 更新 contacts 表：{cursor.rowcount} 行受影响")
    conn.close()
...
def update_helight_company():
    ...
    conn.commit()
    print(f"✓ 更新和光智成公司信息：{cursor.rowcount} 行受影响")
    conn.close()

def update_deepchem_company():
    """更新深云智合公司信息"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    ...
    ''', (
        ...
        'IT 服务/化工/AI 材料',
        '2020-11-09',
        '成立于 2020 年 11 月，注册资本 401.02 万元。量子化学计算云平台运营商，AI 与分子合成深度结合。2023 年与包头九原区合作建设"黑灯实验室"，2024 年 6 月正式投产。存在法律诉讼纠纷（2025 年）。'
    ))
    
    conn.commit()
    print(f"✓ 更新深云智合公司信息：{cursor.rowcount} 行受影响")
    conn.close()
...
if __name__ == '__main__':
    ...
    update_liuyuzhou_contact()
    update_liuyuzhou_entity()
    update_helight_company()
    update_deepchem_company()
