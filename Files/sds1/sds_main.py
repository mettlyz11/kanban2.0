...
class SDSController:
    ...
    def _refresh_db_connection(self):
        ...
        try:
            from lib.db_connector import reset_pool, get_db_connection
    ...
    def run_full_cycle(self) -> Dict:
        ...
        if WORKSPACE_INTEGRATION_AVAILABLE:
            try:
                #                 # 1. 刷新 Workspace 文件索引（每300个周期执行一次，避免过于频繁）
                #                 if self.state['total_cycles'] % 300 == 0:
                #                     logger.info("📁 刷新 Workspace 文件索引...")
                #                     index_result = refresh_workspace_index()
                #                     logger.info(f"   索引完成: {index_result.get('total_files', 0)} 个文件")
                #                 
                # 2. 从近期 Memory 学习教训
